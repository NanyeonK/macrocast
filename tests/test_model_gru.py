"""Unit tests for GRUModel (Phase 5)."""
...
def test_fit_predict_smoke():
    ...
    model = GRUModel(config=DeepModelConfig(seed=42, max_epochs=5)).fit(X, y)
...
def test_seed_reproducibility():
    ...
    yhat1 = GRUModel(config=cfg).fit(X, y).predict_next(series[-12:])
    yhat2 = GRUModel(config=cfg).fit(X, y).predict_next(series[-12:])
    assert yhat1 == yhat2
...
def test_seed_divergence():
    series = _synthetic_ar1()
    X, y = _windows(series, 12, 1)
    yhat_a = GRUModel(config=DeepModelConfig(seed=1, max_epochs=3)).fit(X, y).predict_next(series[-12:])
    yhat_b = GRUModel(config=DeepModelConfig(seed=2, max_epochs=3)).fit(X, y).predict_next(series[-12:])
