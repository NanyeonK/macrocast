...
from macrocast.pipeline import get_model_defaults, validate_model_registry
...
class ResolvedExperimentSpec:
    ...
    def to_contract_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            'tree_context': self.meta_config.get('tree_context', {}),
        }
...
def _validate_meta_registries() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    benchmarks = load_operational_benchmark_registry()
    presets = load_preset_registry()
    global_defaults = load_operational_global_defaults_registry()
    validate_axes_registry(axes)
    validate_benchmark_registry(benchmarks)
    validate_preset_registry(presets, invariant_axes=set(axes['invariant_axes']), known_axes=_known_axes(axes))
...
def resolve_experiment_spec_from_experiment_config(exp_cfg: ExperimentConfig, *, preset_id: str | None = None, experiment_overrides: dict[str, Any] | None = None) -> ResolvedExperimentSpec:
    return _build_resolved_spec(exp_cfg, preset_id=preset_id, experiment_overrides=experiment_overrides)
...
def _build_resolved_spec(exp_cfg: ExperimentConfig, *, preset_id: str | None, experiment_overrides: dict[str, Any] | None) -> ResolvedExperimentSpec:
    ...
    dataset_registry = load_operational_dataset_registry()
    target_registry = load_operational_target_registry()
    data_task_registry = load_operational_data_task_registry()
    model_registry = load_operational_model_registry()
    validate_dataset_registry(dataset_registry)
    validate_target_registry(target_registry)
    validate_data_task_registry(data_task_registry)
    validate_model_registry(model_registry)
