...
def decompose_treatment_effects(
    ...
    data_rich_regs = {"factors", "ridge", "lasso", "adaptive_lasso",
                      "group_lasso", "elastic_net", "tvp_ridge", "booging"}
    model_meta["d_data_rich"] = model_meta["regularization"].isin(data_rich_regs).astype(float)

    # CV scheme: 1 if K-fold (preferred per CLSS 2022)
    model_meta["d_kfold"] = model_meta["cv_scheme"].str.startswith("_KFoldCV").astype(float)
