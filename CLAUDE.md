...
- All models implement MacrocastEstimator interface: `.fit(X, y)`, `.predict(X)`, `.nonlinearity_type`.
- Multi-step forecasting: "direct" trains separate models per horizon. "iterated" uses recursive one-step.
- Expanding window is default. Rolling window is an option.
