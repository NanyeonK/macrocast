...
| `multi_step_raw_panel_payload_v1` | Layer 3 | `owned` | future raw-panel iterated generators | artifact writer and evaluation | `gated_named` | Must distinguish step predictions, final horizon predictions, recursive state, assumed future-X path, and path-level metrics. |
