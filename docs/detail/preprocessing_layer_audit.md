...
| `temporal_feature_block` | `none`, `moving_average_features` | Moving-average temporal features are executable for raw-panel feature builders with trailing 3-period `{predictor}_ma3` features. X-lag/factor composition and other temporal blocks remain registry-only. |
