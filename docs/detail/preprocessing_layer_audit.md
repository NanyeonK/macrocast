...
| `level_feature_block` | `none`, `target_level_addback` | Target-level add-back is executable for raw-panel feature builders. It appends observed `target_t` for training rows and `target_origin` for prediction rows; selected X-level add-backs and level-growth pairs remain registry-only. |
