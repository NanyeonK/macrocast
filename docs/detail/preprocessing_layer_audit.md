...
| `temporal_feature_block` | `none`, `moving_average_features`, `volatility_features` | Moving-average and volatility temporal features are executable for raw-panel feature builders with trailing 3-period `{predictor}_ma3` and `{predictor}_vol3` features. X-lag/factor composition and other temporal blocks remain registry-only. |
