...
| `level_feature_block` | `none`, `target_level_addback`, `x_level_addback` | Level add-backs are executable for raw-panel feature builders. Target add-back appends observed `target_t` / `target_origin`; X-level add-back appends raw-level `H` predictor values preserved before official transforms/T-codes. Selected subset add-backs and level-growth pairs remain registry-only. |
| `temporal_feature_block` | `none`, `moving_average_features`, `rolling_moments`, `volatility_features` | Moving-average, rolling-moment, and volatility temporal features are executable for raw-panel feature builders with trailing 3-period `{predictor}_ma3`, `{predictor}_mean3` / `{predictor}_var3`, and `{predictor}_vol3` features. X-lag/factor composition and other temporal blocks remain registry-only. |
| Other feature-block primitive axes | none | `feature_block_set`, `rotation_feature_block`, and `feature_block_combination` remain registry-only. |
