...
| `level_feature_block` | 2_preprocessing | `none` and `target_level_addback` operational through raw-panel bridge lowering; selected level add-backs and level-growth pairs remain gated |
| `temporal_feature_block` | 2_preprocessing | `none`, `moving_average_features`, and `volatility_features` operational through raw-panel bridge lowering; X-lag/factor composition and other temporal blocks remain gated |
