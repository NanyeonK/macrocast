...
| `rotation_feature_block` | 2_preprocessing | `none` and `moving_average_rotation` operational through raw-panel bridge lowering; MARX has a lag-polynomial naming/alignment contract skeleton but remains registry-only; MAF, custom rotations, and rotation composition remain gated |
